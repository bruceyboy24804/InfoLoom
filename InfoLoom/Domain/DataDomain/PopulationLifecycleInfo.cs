...
{
	public struct PopulationLifecycleInfo : IJsonWritable
	{
		...
		public PopulationLifecycleInfo(int group)
		{
			Group = group;
			Total = 0;
			Work = 0;
		...
		{
			writer.TypeBegin(typeof(PopulationLifecycleInfo).FullName);
			...
			writer.Write(ChildOrTeenWithNoSchool);
			writer.TypeEnd();
		}
	}
}
