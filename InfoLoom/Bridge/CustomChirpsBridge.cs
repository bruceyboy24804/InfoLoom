...
namespace InfoLoomTwo.Bridge
{
    ...
    public static class CustomChirpsBridge
    {
        ...
        private static object MapDepartment(DepartmentAccountBridge department)
        {
            ...
            {
                EnsureResolve();
                if (_deptEnumType == null) return "CensusBureau"; // dead fallback, won't be used if API present
                return Enum.Parse(_deptEnumType, department.ToString(), ignoreCase: false);
            }
            catch
            {
                return Enum.Parse(_deptEnumType, "CensusBureau", ignoreCase: true);
            }
        }
