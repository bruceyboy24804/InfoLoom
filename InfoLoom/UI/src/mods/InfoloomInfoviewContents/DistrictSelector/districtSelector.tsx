...
const bindingDistrictInfos = bindValue<DistrictInfo[]>(mod.id, 'districtInfos');
const bindingSelectedDistrict = bindValue<Entity>(mod.id, 'selectedDistrict', {
  index: 0,
  version: 0,
});
...
export const DistrictSelector = () => {
  ...
  const districtDropdownItems: JSX.Element[] = districtInfos.map((districtInfo: DistrictInfo) => {
    ...
    return (
      <DropdownItem
        ...
        onChange={() => trigger(mod.id, 'selectedDistrictChanged', districtEntity)}
      >
        {districtName}
      </DropdownItem>
    );
    ...
    <ModuleResolver.instance.Tooltip
      ...
      tooltip={
        <ModuleResolver.instance.FormattedParagraphs
          children={'Set whether to show for entire city or a specific district.'}
        />
      }
      ...
      children={
        <div className={styles.districtDropdownRow}>
          <Dropdown theme={ModuleResolver.instance.DropdownClasses} content={districtDropdownItems}>
            <DropdownToggle>{selectedDistrictName}</DropdownToggle>
          </Dropdown>
        </div>
      }
    />
