...
const PipelineChartComponent: React.FC<PipelineChartProps> = ({ isWorkplaceMode, groupBySector }) => {
        ...
        plugins: {
          legend: {
            ...
            labels: {
              ...
              padding: 12,
              generateLabels: () =>
                legendItemsRef.current.map(item => {
  ...
  useEffect(() => {
    ...
    if (!isWorkplaceMode) {
      ...
      [...AGE_NAMES, ...EDU_NAMES, ...NON_WORK_NAMES, ...workerNames].forEach(n => {
        labels[n] = n;
      });
      ...
      LIVING_NAMES.forEach(k => {
        labels[k] = k.replace('L: ', '');
      });
      WP_EDU_NAMES.forEach((k, i) => {
        labels[k] = EDU_NAMES[i];
      });
      JOB_EDU_NAMES.forEach((k, i) => {
        labels[k] = EDU_NAMES[i];
      });
      WP_SECTOR_NAMES.forEach(k => {
        labels[k] = k.replace('S: ', '');
      });
    ...
    const hidden = hiddenRef.current;
    const visible = hidden.size > 0 ? flows.filter(f => !hidden.has(f.from) && !hidden.has(f.to)) : flows;
    flowsRef.current = visible;
...
const WorkforcePipelineSankey: React.FC<DraggablePanelProps> = ({ onClose, initialPosition }) => {
  ...
  return (
    ...
    >
      ...
      <div className={styles.columnHeaders}>{isWorkplaceMode ? workplaceColumns : workforceColumns}</div>
